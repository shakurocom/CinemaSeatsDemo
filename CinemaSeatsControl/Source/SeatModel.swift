...
class SeatModel {
    ...
    static func generate(for time: Time) -> [SeatModel] {
        ...
        let a = SeatModel(type: .available, image: UIImage.loadImageFromBundle(name: "availableSeat"), currency: 10)
        let u = SeatModel(type: .unavailable, image: UIImage.loadImageFromBundle(name: "unavailableSeat"), currency: 10)
